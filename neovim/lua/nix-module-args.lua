...
function M.prune_empty_arg(bufnr)
  ...
  -- No reformat needed afterward: we're deleting a clean, well-defined
  -- range (the header, exactly as _skel inserted it) and not introducing
  -- any new text, so what's left is already properly formatted.
  local body = w.last_fn:field("body")[1]
  local bs_row, bs_col = body:range()
  local ns = vim.api.nvim_create_namespace("nix_module_args")

  -- Track the window cursor across the deletion: it may currently sit
  -- inside `body` (e.g. right after a luasnip jump to _skel's final
  -- tabstop), and a plain (row, col) captured now would go stale the
  -- moment the header above it disappears -- Neovim doesn't otherwise
  -- know to shift *this* window's cursor for an edit made from outside
